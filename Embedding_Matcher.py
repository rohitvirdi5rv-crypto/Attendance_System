...
def mark_attendance_logic(emp_id, emp_name):
    ...
    # Time ranges
    in_start = datetime.strptime("01:46", "%H:%M").time()
    in_end   = datetime.strptime("01:48", "%H:%M").time()

    half_start = datetime.strptime("01:50", "%H:%M").time()
    half_end   = datetime.strptime("01:53", "%H:%M").time()

    out_start = datetime.strptime("01:55", "%H:%M").time()
    ...
    # --------------------------------
        ...
        if mask.any():
            ...
            # Rule: If half day already taken → OUT not allowed
